% \documentclass{beamer}
\documentclass[aspectratio=169]{beamer}
\usepackage{hyperref}
%% \usepackage[CJKmath=true, AutoFakeBold = true]{xeCJK}
\usepackage[T1]{fontenc}
%% \setCJKmainfont{AR PL KaitiM GB} % Set Chinese font to KaiTi (calligraphy style)

% Other packages
\usepackage{latexsym,xcolor,multicol,booktabs,calligra}
% \usepackage{amssymb,amsfonts,amsmath,amsthm,mathrsfs,mathptmx} % Common math packages
\usepackage{amsmath,amssymb,amsfonts,mathrsfs} % Best mathematical packages
\usepackage{newtxtext,newtxmath} % Times-like font package for text and mathematics

\usepackage{graphicx,pstricks,listings,stackengine}
\usefonttheme[onlymath]{serif} % Use serif font for equations (better appearance)

% Other packages (new)
\usepackage[linesnumbered,ruled,vlined]{algorithm2e}
\SetKwComment{Comment}{$\triangleright$ }{} % The argument will be surrounded by /* ... */
\SetAlCapFnt{\footnotesize} % Make caption text smaller
\SetAlCapNameFnt{\footnotesize} % Make "Algorithm" label smaller
\usepackage{graphicx} % Enables inclusion and scaling of images (\includegraphics)
\usepackage{animate} % Provides support for frame-by-frame animations (\animategraphics)
\usepackage{subcaption} % Provides support for subfigures (subfigure environment with captions)
\usepackage{fontawesome5} % Provides scalable vector icons (Font Awesome 5 icons)
\usepackage{caption}
\captionsetup{
    font=footnotesize, % Reduce caption text size
    skip=5pt, % Space between figure and caption
    belowskip=0pt % Space below the caption (reduces gap after caption)
}
\usepackage{forest}
\usepackage[table]{xcolor}

% Adjusts horizontal margins of the slide content
% \setbeamersize{
%     text margin left=20pt, % Left margin of the slide content
%     text margin right=20pt % Right margin of the slide content
% }

\newcommand{\source}[1]{\par\tiny Source: #1} % Defines a reusable command to display the image/table source in small font below the caption

\renewcommand{\today}{\number\year-\number\month-\number\day}
\renewcommand{\alert}[1]{\textbf{\color{swufe}#1}}
\newcommand{\alertplain}[1]{{\color{swufe}#1}}

% Title page setup
% \author{Lu** \inst{1}, Zhang** \inst{1}}
\author[Topics]{Topics}

\title[Title of Presentation]{Title of Presentation}
% Custom title template with a styled box
\setbeamertemplate{title}{
    \makebox[\textwidth]{ % force horizontal centering
        \begin{beamercolorbox}[
            wd=1.0\textwidth,
            sep=1pt,
            center,
            rounded=true,
            shadow=false
        ]{title}
            \usebeamerfont{title}
            \inserttitle
        \end{beamercolorbox}
    }
}

% \subtitle{Jiangnan University Academic Report Beamer Template}
%% \institute[JNU]{\normalsize{\inst{1} School of Business \quad Jiangnan University \quad \textit{****@qq.com}}}
\institute[Session 1]{
    \includegraphics[width=0.3\linewidth]{pic/logo_graph.png}
}

% \date{\today}
\date{\scriptsize March, 2026}

\usepackage{JNU}

% Defs
\def\cmd#1{\texttt{\color[RGB]{0, 0, 139}\footnotesize $\backslash$#1}}
\def\env#1{\texttt{\color[RGB]{0, 0, 139}\footnotesize #1}}

\lstset{
    language=[LaTeX]TeX,
    basicstyle=\ttfamily\footnotesize,
    keywordstyle=\bfseries\color[RGB]{0, 0, 139},
    stringstyle=\color[RGB]{50, 50, 50},
    numbers=left,
    numberstyle=\small\color{gray},
    rulesepcolor=\color{red!20!green!20!blue!20},
    frame=shadowbox,
}

\begin{document}

\begin{frame}
    \titlepage
    \begin{figure}[htpb]
        \begin{center}
            %% \includegraphics[width=0.55\linewidth]{pic/jiangnan_logo.png}
        \end{center}
    \end{figure}
\end{frame}

\begin{frame}
    \tableofcontents[sectionstyle=show,subsectionstyle=show/shaded/hide,subsubsectionstyle=show/shaded/hide]
    % \tableofcontents[sectionstyle=show,subsectionstyle=hide,subsubsectionstyle=hide]
\end{frame}

\section{Components}
    \subsection{Lists}
        \begin{frame}{Simple list}
            \begin{itemize}
                \item Google
                \item Microsoft
                \item Apple
                \item Amazon
                \item Meta
            \end{itemize}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Enumerate list}
            \begin{enumerate}
                \item \texttt{Python}
                \item \texttt{Java}
                \item \texttt{C++}
                \item \texttt{C}
                \item \texttt{Go}
            \end{enumerate}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{List of list (1)}
            \begin{itemize}
                \item Install requirements
                    \begin{itemize}
                        \item \texttt{Python}
                        \item \texttt{Numpy}
                        \item \texttt{Pandas}
                        \item \texttt{NetworkX}
                        \item \texttt{Matplotlib}
                    \end{itemize}
                \item Topics 
                    \begin{itemize}
                        \item Create graphs
                        \item Graphs visualization
                    \end{itemize}
                \item Links
                    \begin{itemize}
                        \item \href{https://github.com/win7}{
                                \beamergotobutton{\faGithub\ Open in GitHub}
                            }
                        \item \href{https://colab.research.google.com/}{
                                \beamergotobutton{\faGoogle\ Open in Google Colab}
                            }
                    \end{itemize}
            \end{itemize}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{List of list (2)}
            \begin{itemize}
                \item Word embeddings
                \begin{itemize}
                    \item Word2vec
                    \begin{itemize}
                        \item \alert{Skip-Gram}
                        \item Continuous Bag of Words (CBOW)
                    \end{itemize}
                \end{itemize}
                \item Graph embeddings
                \begin{itemize}
                    \item Shallow embeddings
                    \begin{itemize}
                        \item DeepWalk
                        \item \alert{Node2vec}
                        \item Struc2vec
                        \item Structural Deep Network Embedding (SDNE)
                        \item Metapath2vec
                    \end{itemize}
                    \item Graph Neural Networks (GNN)
                    \begin{itemize}
                        \item Graph Convolutional Networks (GCN)
                        \item Graph Attention Networks (GAT)
                        \item Graph Isomorphism Networks (GIN)
                        \item GraphSAGE
                        \item Graph Autoencoders (GAE)
                        % \item Graph Transformers
                    \end{itemize}
                \end{itemize}
            \end{itemize}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Multicols list}
            \begin{multicols}{2} % Two columns
                \begin{itemize}
                    \item Computer vision
                    \item Natural language processing
                    \item Recommender systems
                    \item Program analysis
                    \item Knowledge graphs
                    \item Bioinformatics
                    \item Chemistry
                    \item Biology
                    \item Physic
                    \item Anomaly detection
                    \item Urban intelligence
                    \item Financial transaction
                \end{itemize}
            \end{multicols}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}
        
        \begin{frame}{Description list}
            \begin{description}
                \item[Input:] Graph data
                \item[Output:] Community labels
                \item[Method:] Leiden algorithm
            \end{description}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Sequential list display}
            \begin{itemize}[<+-| alert@+>]
                \item Many people use \LaTeX{}, and many universities have their own Beamer themes
                \item For Chinese support, use Xe\LaTeX{} as the compiler
            \end{itemize}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}[allowframebreaks]{List (dynamic)}
            \begin{itemize}
                \setlength{\itemsep}{10pt}
                \item This template is modified from a Tsinghua University project
                \item Original template: \url{https://www.overleaf.com/latex/templates/thu-beamer-theme/vwnqmzndvwyb}
                \item Minor modifications were made
                \item Style adjusted to personal preference
                \item Added small Beamer usage examples
                \item This is \emph{emphasized text}
                \item Replaced default fonts with more readable ones
                \item Fixed inconsistent Chinese font issues
                \item Changed math fonts to serif for better appearance
                \item \alert{Use \cmd{alert} to emphasize text}
                \item This template is modified from a Tsinghua University project
                \item Original template: \url{https://www.overleaf.com/latex/templates/thu-beamer-theme/vwnqmzndvwyb}
            \end{itemize}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

    \subsection{Maths}
        \begin{frame}{Simple equations}
            $V = \frac{4}{3} \pi r^3$

            \[
                V = \frac{4}{3} \pi r^3
            \]

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Equations}
            \begin{equation*}
                E = mc^2
                % \label{eq:equation1}
            \end{equation*}

            \begin{equation}
                J(\theta) = \mathbb{E}_{\pi_\theta}[G_t]
                % \label{eq:equation1}
            \end{equation}

            \begin{align}
                Q_{\mathrm{target}} &= r + \gamma Q^\pi(s', \pi_\theta(s') + \epsilon)\\
                \epsilon &\sim \mathrm{clip}(\mathcal{N}(0, \sigma), -c, c)
                % \label{eq:equation1}
            \end{align}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Equation with numbers}
            % Taken from Mathmode.tex
            \begin{multline}
                A=\lim_{n\rightarrow\infty}\Delta x\left(a^{2}+\left(a^{2}+2a\Delta x+\left(\Delta x\right)^{2}\right)\right. \\
                +\left(a^{2}+2\cdot2a\Delta x+2^{2}\left(\Delta x\right)^{2}\right) \\
                +\left(a^{2}+2\cdot3a\Delta x+3^{2}\left(\Delta x\right)^{2}\right) \\
                +\ldots \\
                \left.+\left(a^{2}+2\cdot(n-1)a\Delta x+(n-1)^{2}\left(\Delta x\right)^{2}\right)\right) \\
                =\frac{1}{3}\left(b^{3}-a^{3}\right)
                % \label{eq:equation1}
            \end{multline}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Algorithms}
            \begin{algorithm}[H]
                \small % \scriptsize \footnotesize \small
                \DontPrintSemicolon
                % \SetAlgoLined
                % \SetNoline
                
                \SetKwInOut{Input}{Input}
                \SetKwInOut{Output}{Output}
                
                \Input{$G$, $s$ \Comment*[r]{Graph $G$ and a source node $s$}}
                \Output{All nodes reachable from $s$}
                
                % \BlankLine
                $Q \gets \varnothing$ \Comment*[r]{Create an empty queue}
                $V \gets \varnothing$ \Comment*[r]{Create an empty set of visited}
                $enqueue(Q, s)$\;
                $append(V, s)$\;
                
                \While{$Q \neq \varnothing$}{
                    $v \gets dequeue(Q)$\;
                    $print(v)$\;

                    \ForEach{$u \in N(v)$}{ % \Comment*[r]{process all neighbors of node $u$}
                        \If{$u \notin V$}{
                            $append(V, u)$\;
                            $enqueue(Q, u)$
                        }
                    }
                }
                % \Return max\;
                \caption{My algorithm.}
                % \label{tab:algorithm}
            \end{algorithm}
            
            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

    \subsection{Tables}
        \begin{frame}{Table}
            \begin{table}[htp!]
                % \scriptsize
                \centering
                \caption{My table.}
                % \label{tab:table1}
                \begin{tabular}{ll}\toprule
                    Representation & Complexity \\\midrule
                    Adjacency matrix & $O(|V|^2)$ \\
                    Adjacency list & $O(|V| + |E|)$ \\
                    Edge list & $O(|E|)$ \\
                    Incidence matrix & $O(|V||E|)$ \\\bottomrule
                \end{tabular}
            \end{table}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Table (custom)}
            \begin{table}[htp!]
                \scriptsize
                \centering
                \caption{My table.}
                % \label{tab:table1}
                \begin{tabular}{p{2.2cm}p{4cm}p{4cm}}\toprule
                    Representation & Advantages & Disadvantages \\\midrule
                    Adjacency matrix & Simple structure, constant-time edge lookup. & High memory usage for large sparse graphs. \\
                    Adjacency list & Memory efficient for sparse graphs, easy to iterate over neighbors. & Edge existence check may be slower. \\
                    Edge list & Very simple representation, useful for algorithms that process edges sequentially. & Inefficient for neighbor queries.  \\
                    Incidence matrix & Useful in some theoretical formulations and algebraic graph analysis. & High memory usage and rarely used in practical implementations. \\\bottomrule
                \end{tabular}
            \end{table}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Table (custom)}
            \begin{table}[htp!]
                % \scriptsize
                \centering
                \caption{My table.}
                % \label{tab:table1}
                \begin{tabular}{c|c}
                    IA & Tasks \\
                    \hline
                    AI learns patterns & AI automates tasks \\
                    AI processes data & AI supports decisions \\
                    AI powers assistants & AI transforms industries \\
                    AI evolves & AI predicts \\
                \end{tabular}
            \end{table}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

    \subsection{Figures}
        \begin{frame}{Figure}
            \begin{figure}[htp!]
                \centering
                \includegraphics[
                    width=0.3\linewidth,
                    trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                    clip
                ]{pic/default_image.png}
                \caption{My image.}
                \source{Author, 2024.}
                % \label{fig:my_image}
            \end{figure}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Multiple figures 1}
            \begin{figure}
                \centering
                \begin{subfigure}{0.2\textwidth}
                    \centering
                    \includegraphics[
                        width=\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    % \source{Author, 2024.}
                \end{subfigure}
                \hfill
                \begin{subfigure}{0.2\textwidth}
                    \centering
                    \includegraphics[
                        width=\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    % \source{Author, 2024.}
                \end{subfigure}
                \hfill
                \begin{subfigure}{0.2\textwidth}
                    \centering
                    \includegraphics[
                        width=\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    % \source{Author, 2024.}
                \end{subfigure}
                \hfill
                \begin{subfigure}{0.2\textwidth}
                    \centering
                    \includegraphics[
                        width=\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    % \source{Author, 2024.}
                \end{subfigure}
                \caption{My image.}
                \source{Author, 2024.}
                % \label{fig:my_image}
            \end{figure}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Multiple figures 2}
            \begin{figure}
                \centering
                % Left column: contains two vertically stacked subfigure
                \begin{minipage}{0.5\textwidth}
                    \centering
                    \begin{subfigure}{\textwidth}
                        \centering
                        \includegraphics[
                            width=0.3\linewidth,
                            trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                            clip
                        ]{pic/default_image.png}
                        \caption{My image.}
                        \source{Author, 2024.}
                        % \label{fig:my_image}
                    \end{subfigure}
                    \vspace{0.5em}
                    \begin{subfigure}{\textwidth}
                        \centering
                        \includegraphics[
                            width=0.3\linewidth,
                            trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                            clip
                        ]{pic/default_image.png}
                        \caption{My image.}
                        \source{Author, 2024.}
                        % \label{fig:my_image}
                    \end{subfigure}
                \end{minipage}
                \hfill
                % Right column: contains one larger subfigure
                \begin{minipage}{0.45\textwidth}
                    \centering
                    \begin{subfigure}{\textwidth}
                        \centering
                        \includegraphics[
                            width=0.8\linewidth,
                            trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                            clip
                        ]{pic/default_image.png}
                        \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                    \end{subfigure}
                \end{minipage}
                \caption{My image.}
                \source{Author, 2024.}
                % \label{fig:my_image}
            \end{figure}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Figure (dynamic)}
            \begin{figure}
                \centering
                \animategraphics[width=\textwidth,loop,autoplay]
                {5} % frame rate
                {pic/imd/freiburg_matches-} % path to figures
                {0} % start index
                {15} % end index
                \caption{My image.}
                \source{Author, 2024.}
                % \label{fig:my_image}
            \end{figure}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

    \subsection{Others}
        \begin{frame}{Blocks 1}
            \begin{block}{Info}
                General content
            \end{block}

            \begin{alertblock}{Warning}
                Important message
            \end{alertblock}

            \begin{exampleblock}{Example}
                Example content
            \end{exampleblock}

            \begin{definition}
                Example definition
            \end{definition}

            \begin{example}
                Example content
            \end{example}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Blocks 2}
            \begin{theorem}[Pythagoras] 
                $a^2 + b^2 = c^2$
            \end{theorem}

            \begin{corollary}
                $x + y = y + x$
            \end{corollary}

            \begin{proof}
                $\omega + \phi = \epsilon$
            \end{proof}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Frame (two columns)}
            \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
                \begin{column}{0.5\textwidth}
                    \hspace{0.2cm} Se enfoca solo en:
                    \begin{itemize}
                        \item Números (notas)
                        \item Operaciones matemáticas
                    \end{itemize}
                \end{column}
                
                \begin{column}{0.5\textwidth}
                    Se ignora:
                    \begin{itemize}
                        \item Nombres de cursos
                        \item Detalles del estudiante
                    \end{itemize}
                \end{column}
            \end{columns}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Frame (two columns (list of list))}
            \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
                \begin{column}{0.45\textwidth}
                    \begin{itemize}
                        \item \alert{Graph traversal}
                        \begin{itemize}
                            \item Breadth-First Search
                            \item Depth-First Search
                        \end{itemize}
                    \end{itemize}
                \end{column}
                \begin{column}{0.55\textwidth}
                    \begin{itemize}
                        \item \alert{Shortest path algorithms}
                        \begin{itemize}
                            \item Dijkstra
                            \item Floyd–Warshall
                            \item Bellman-Ford
                        \end{itemize}
                    \end{itemize}
                \end{column}
            \end{columns}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Frame (three columns)}
            \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
                \begin{column}{0.33\textwidth}
                    \begin{itemize}
                        \item \alert{Node-level}
                        \begin{itemize}
                            \item Degree
                            \item Degree centrality
                            \item Closeness centrality
                            \item Betweenness centrality
                            % \item Centrality
                            \item Eccentricity
                        \end{itemize}
                    \end{itemize}
                \end{column}
                \begin{column}{0.33\textwidth}
                    \begin{itemize}
                        \item \alert{Edge-level}
                        \begin{itemize}
                            \item Shortest path
                            \item Betweenness centrality
                            \item Distance
                        \end{itemize}
                    \end{itemize}
                \end{column}
                \begin{column}{0.33\textwidth}
                    \begin{itemize}
                        \item \alert{Graph-level}
                        \begin{itemize}
                            \item Order
                            \item Size
                            \item Density
                            \item Diameter
                            \item Radius
                            % \item Average path
                            \item Clustering coefficient
                            \item Modularity
                        \end{itemize}
                    \end{itemize}
                \end{column}
            \end{columns}

            \begin{note}
                {Introduce your self}
            \end{note}
        \end{frame}

        \begin{frame}{Tree}
            \begin{center}
                \begin{forest}
                    for tree={
                        grow'=south,        % Top-to-bottom tree layout
                        draw,               % Draw node borders
                        rounded corners,    % Rounded box style
                        align=center,       % Center node text
                        edge={->},          % Directed edges with arrows
                        l sep=15pt,         % Vertical spacing between levels
                        s sep=12pt,         % Horizontal spacing between siblings
                        inner sep=6pt,      % Internal padding of nodes
                        fill=blue!10        % Light blue background for nodes
                    }
                    [Graph Machine Learning
                        [Representation Learning
                            [Graph Embeddings
                                [Shallow Methods
                                    [DeepWalk]
                                    [Node2Vec]
                                    [LINE]
                                ]
                                [GNN-based
                                    [GCN]
                                    [GAT]
                                    [GraphSAGE]
                                ]
                            ]
                        ]
                    ]
                \end{forest}
            \end{center}

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

        \begin{frame}{Footnote}
            GAT: Graph Attention Networks \footnote{Veličković, P., Cucurull, G., Casanova, A., Romero, A., Lio, P., \& Bengio, Y. (2017). Graph attention networks. arXiv preprint arXiv:1710.10903.}. \newline

            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce luctus pulvinar ante, sed vehicula massa facilisis luctus. Morbi feugiat molestie mauris, sollicitudin efficitur nulla malesuada ut. Nunc a augue rutrum, pharetra ex a, mollis nibh. Nam eget ultrices risus. Sed rutrum tincidunt viverra. Nullam ultricies mi leo, sit amet tincidunt elit hendrerit ac. Sed aliquam pellentesque felis. Suspendisse potenti. Nam a lacus sapien. Maecenas finibus vulputate rhoncus. Sed bibendum porta velit eget gravida. Donec vel ornare arcu. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. 

            \begin{note}
                {Write your notes here}
            \end{note}
        \end{frame}

\section{Examples}
    \begin{frame}{Example 1}
        In mathematics and computer science, a graph (network) is defined as $G = (V, E)$, where $V = \{v_1, v_2, ..., v_{n}\}$ is the nodes (or \textit{vertices}, or \textit{points}) set and $E = \{e_1, e_2, ..., e_{m}\}, e_i \in V \times V$ a set of edges (or \textit{lines}, or \textit{arcs}) formed by unordered pairs of nodes. \newline

        \scriptsize Number of nodes is denoted $|V|$. \\
        Number of edges is denoted $|E|$.

        \begin{figure}[htp!]
            \centering
            \includegraphics[
                width=0.2\linewidth,
                trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                clip
            ]{pic/default_image.png}
            \caption{My image.}
            \source{Author, 2024.}
            % \label{fig:my_image}
        \end{figure}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 2}
        Furthermore, each edge $e \in E$ is said to join two vertices. If $e$ joins $u, v \in V$, then $e = (u, v)$. Vertices $u$ and $v$ are said to be adjacent.
        
        \begin{alertblock}{Neighborhood}
            \begin{equation}
                N(v) = \{w \in V \mid v \neq w \land (v, w) \in E\}
                % \label{eq:my_equation}
            \end{equation}
        \end{alertblock}

        \alert{Backward propagation} computes the gradients of the loss $\mathcal{L}$ or $\mathscr{L}$ and updates the parameters using gradient descent.
        \begin{align}
            W^{(l)} = W^{(l)} - \eta \frac{\partial \mathcal{L}}{\partial W^{(l)}} \\
            b^{(l)} = b^{(l)} - \eta \frac{\partial \mathcal{L}}{\partial b^{(l)}}
        \end{align}

        \alert{Sum operator}: $z_{uv} = z_u + z_v \in \mathbb{R}^d$.

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 3}
        \begin{alertblock}{Adjacency matrix}
            Adjacency matrix \textbf{$A$}, is an $n \times n$ matrix where: 
            \[
                A_{i, j} =
                \begin{cases}
                1 & \text{if there is an edge from vertex } i \text{ to vertex } j \\
                0 & \text{otherwise}
                \end{cases}
            \]
        \end{alertblock}

        \begin{figure}
            \centering
            \begin{subfigure}{0.15\textwidth}
                \centering
                \includegraphics[
                    width=\linewidth,
                    trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                    clip
                ]{pic/default_image.png}
                \caption{My image.}
                % \source{Author, 2024.}
            \end{subfigure}
            % \hfill
            \hspace{1cm}
            \begin{subfigure}{0.15\textwidth}
                \centering
                \includegraphics[
                    width=\linewidth,
                    trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                    clip
                ]{pic/default_image.png}
                \caption{My image.}
                % \source{Author, 2024.}
            \end{subfigure}
            \caption{My image.}
            \source{Author, 2024.}
            % \label{fig:my_image}
        \end{figure}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}{Example 4}
        \begin{alertblock}{Idea}
            Does the shortest path from $a$ to $c$ go through $b$?
        \end{alertblock}

        \begin{figure}[htp!]
            \centering
            \includegraphics[
                width=0.3\linewidth,
                trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                clip
            ]{pic/default_image.png}
            \caption{My image.}
            \source{Author, 2024.}
            % \label{fig:my_image}
        \end{figure}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 5}
        BFS is a graph traversal algorithm that explores nodes level by level, visiting all nodes at a given distance from the starting node before moving to the next level. \newline

        Description:
        \begin{itemize}
            \item Uses a queue (FIFO) to manage the nodes to be explored.
            \item Marks visited nodes to avoid cycles or repeated visits.
            \item Ideal for finding the shortest path in unweighted graphs.
        \end{itemize}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 6}
        In this lab, students will apply basic programming concepts to solve problems, develop coding skills, and practice writing and debugging efficient solutions. \newline

        \begin{itemize}
            \item Requirements
                \begin{itemize}
                    \item \texttt{Python}
                    \item \texttt{Numpy}
                    \item \texttt{Pandas}
                    \item \texttt{NetworkX}
                    \item \texttt{Matplotlib}
                \end{itemize}
            \item Topics 
                \begin{itemize}
                    \item Create graphs
                    \item Graphs visualization
                \end{itemize}
            \item Lab
                \begin{itemize}
                    \item \href{https://colab.research.google.com/}{
                            \beamergotobutton{\faGoogle\ Open in Google Colab}
                        }
                \end{itemize}
        \end{itemize}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}{Example 7}
        This section introduces key equations to model and analyze the problem.

        \begin{alertblock}{Main equation}
            \begin{equation*}
                J(\theta) = \mathbb{E}_{\pi_\theta}[G_t]
                % \label{eq:equation1}
            \end{equation*}
        \end{alertblock}

        \begin{alertblock}{Supplementary equations}
            \begin{align}
                Q_{\mathrm{target}} &= r + \gamma Q^\pi(s', \pi_\theta(s') + \epsilon)\\
                \epsilon &\sim \mathrm{clip}(\mathcal{N}(0, \sigma), -c, c)
                % \label{eq:equation1}
            \end{align}
        \end{alertblock}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}
        \begin{alertblock}{Solution}
            % Taken from Mathmode.tex
            \begin{multline}
                A=\lim_{n\rightarrow\infty}\Delta x\left(a^{2}+\left(a^{2}+2a\Delta x+\left(\Delta x\right)^{2}\right)\right.\\
                +\left(a^{2}+2\cdot2a\Delta x+2^{2}\left(\Delta x\right)^{2}\right)\\
                +\left(a^{2}+2\cdot3a\Delta x+3^{2}\left(\Delta x\right)^{2}\right)\\
                +\ldots\\
                \left.+\left(a^{2}+2\cdot(n-1)a\Delta x+(n-1)^{2}\left(\Delta x\right)^{2}\right)\right)\\
                =\frac{1}{3}\left(b^{3}-a^{3}\right)
                % \label{eq:equation1}
            \end{multline}
        \end{alertblock}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}{Example 8}
        \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
            \begin{column}{0.6\textwidth}
                \begin{enumerate}
                    \item Visit the adjacent unvisited node. Mark as visited, display, and insert (queue) in a queue.
                    \item In case no adjacent node is found, remove the first node from the queue.
                    \item Repeat steps 1, and 2 until the queue is empty.
                \end{enumerate}
            \end{column}
            \begin{column}{0.4\textwidth}
                \begin{figure}[htp!]
                    \centering
                    \includegraphics[
                        width=0.6\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{figure}
            \end{column}
        \end{columns}
        
        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 9}
        The following table summarizes the key information and results, providing a clear overview of the main variables and their relationships.

        \begin{table}[htp!]
            \centering
            \caption{My table.}
            % \label{tab:table1}
            \begin{tabular}{ll}\toprule
                Representation & Complexity \\\midrule
                Adjacency matrix & $O(|V|^2)$ \\
                Adjacency list & $O(|V| + |E|)$ \\
                Edge list & $O(|E|)$ \\
                Incidence matrix & $O(|V||E|)$ \\\bottomrule
            \end{tabular}
        \end{table}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}{Example 10}
        The following table summarizes the key information and results, providing a clear overview of the main variables and their relationships.

        \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
            \begin{column}{0.6\textwidth}
                \begin{table}[htp!]
                    \centering
                    \caption{My table.}
                    % \label{tab:table1}
                    \begin{tabular}{ll}\toprule
                        Representation & Complexity \\\midrule
                        Adjacency matrix & $O(|V|^2)$ \\
                        Adjacency list & $O(|V| + |E|)$ \\
                        Edge list & $O(|E|)$ \\
                        Incidence matrix & $O(|V||E|)$ \\\bottomrule
                    \end{tabular}
                \end{table}
            \end{column}
            \begin{column}{0.4\textwidth}
                \begin{figure}[htp!]
                    \centering
                    \includegraphics[
                        width=0.6\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{figure}
            \end{column}
        \end{columns}
        
        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 11}
        The following table summarizes the key information and results, providing a clear overview of the main variables and their relationships.

        \begin{columns}[c] % Option "t" centers vertically, while "t" aligns to the top.
            \begin{column}{0.6\textwidth}
                \begin{enumerate}
                    \item \alert{Análisis:} Definir requisitos y comprender el problema.
                    \item \alert{Diseño:} Planificar la estructura del algoritmo.
                    \item \alert{Implementación:} Escribir el código en un lenguaje de programación.
                    \item \alert{Prueba:} Verificar el funcionamiento y corregir errores.
                \end{enumerate}
            \end{column}
            \begin{column}{0.4\textwidth}
                \begin{figure}[htp!]
                    \centering
                    \includegraphics[
                        width=0.8\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{figure}
            \end{column}
        \end{columns}
        
        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 12}
        \begin{exampleblock}{Degree}
            Given a graph $G = (V, E)$.

            The degree of a node $u$, denoted as $deg(u)$, is defined as the number of edges that are adjacent to this node $u$.
        \end{exampleblock}

        \begin{multicols}{2} % Two columns
            For undirected graphs:
            \begin{equation}
                deg(u) = \left|\{v \in V : (u, v) \in E\}\right|
                % \label{eq:my_equation}
            \end{equation}
        
            For directed graphs:
            \begin{equation}
                deg(u) = deg^{in}(u) + deg^{out}(u)
                % \label{eq:my_equation}
            \end{equation}
        \end{multicols}
        
        Where, $deg^{in}(u)$: in-degree and $deg^{out}(u)$: out-degree. \newline

        Applications:
        \begin{itemize}
            \item Social networks: users with many connections.
            \item Biology networks: metabolites involved in many reactions.
            \item Web: most linked pages.
        \end{itemize}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 13}
        \begin{exampleblock}{Degree centrality}
            Given a graph $G = (V, E)$.
            
            Degree centrality considers the node with the highest degree as the most central node in the network $G$. Degree centrality focuses on individual nodes; it simply counts the number of edges that a node has.
        \end{exampleblock}

        \begin{equation}
            DC(v) = \frac{deg(v)}{|V| - 1}
            % \label{eq:my_equation}
        \end{equation}
        
        Applications:
        \begin{itemize}
            \item Social networks: who has the most friends?
            \item Internet: which routers or servers handle the most connections?
            % \item Epidemiology: who is most likely to spread information or disease?
            \item Citation networks: which papers are most cited?
        \end{itemize}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 14}
        \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
            \begin{column}{0.3\textwidth}
                \begin{figure}[htp!]
                    \centering
                    \includegraphics[
                        width=1.0\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{figure}
            \end{column}
            \begin{column}{0.7\textwidth}
                
            \end{column}
        \end{columns}
        
        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 15}
        \begin{exampleblock}{Descomposición}
            Consiste en \alert{dividir} el problema en sub problemas o en partes más pequeñas (unidades lógicas de menor complejidad); luego, solucionar cada sub problema independientemente. Finalmente, las soluciones a los subproblemas se integran para resolver el problema original completo. \\
        \end{exampleblock}

        \alertplain{Ejemplo:} Sistema para el cálculo de promedio de notas. \newline

        Dividir el problema en:
        \begin{enumerate}
            \item Ingresar notas
            \item Calcular suma
            \item Calcular promedio
            \item Mostrar resultado
        \end{enumerate}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 16}
        \begin{exampleblock}{Abstracción}
            Consiste en la capacidad de identificar los aspectos más importantes de un problema o situación y reducir la complejidad al enfocarse en las partes más relevantes, ignorando los detalles innecesarios. En otras palabras, abstraer implica crear una \alert{representación  simplificada} de un problema, modelo o sistema, lo cual facilita su comprensión, análisis y resolución. \\
        \end{exampleblock}

        \alertplain{Ejemplo:} Sistema para el cálculo de promedio de notas. \newline

        \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
            \begin{column}{0.5\textwidth}
                \hspace{0.2cm} Se enfoca solo en:
                \begin{itemize}
                    \item Números (notas)
                    \item Operaciones matemáticas
                \end{itemize}
            \end{column}
            
            \begin{column}{0.5\textwidth}
                Se ignora:
                \begin{itemize}
                    \item Nombres de cursos
                    \item Detalles del estudiante
                \end{itemize}
            \end{column}
        \end{columns}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 17}
        \begin{exampleblock}{Diseño de algoritmo}
            Es la capacidad de descomponer un problema en una secuencia de pasos lógicos o instrucciones claras que pueden ser ejecutadas para solucionar un problema. En otras palabras, es el proceso de \alert{crear}, \alert{diseñar} y \alert{evaluar} algoritmos que resuelven problemas de manera eficiente. \\  % Es un componente central del pensamiento computacional, pero también es una habilidad útil en diversas disciplinas. \\
        \end{exampleblock}

        \alertplain{Ejemplo:} Sistema para el cálculo de promedio de notas. \newline

        \begin{algorithm}[H]
            % \scriptsize % \footnotesize
            \DontPrintSemicolon
            % \SetAlgoLined
            % \SetNoline
            
            \SetKwInOut{Input}{Input}
            \SetKwInOut{Output}{Output}
            
            \Input{$nota_1$, $nota_2$ y $nota_3$ \Comment*[r]{Notas del estudiante}}
            \Output{$promedio$}
            
            % \BlankLine
            $suma \gets nota_1 + nota_2 + nota_3$
            
            $promedio \gets \frac{suma}{3}$ 
            \Comment*[r]{Cálculo del promedio}

            $\text{Mostrar} \: promedio$
            % \Return max\;
            \caption{Promedio de notas.}
            % \label{tab:algorithm}
        \end{algorithm}
    
        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

    \begin{frame}{Example 18}
        \begin{exampleblock}{Secuenciales}
            En este tipo de estructura las instrucciones se realizan o se ejecutan una después de la otra.
        \end{exampleblock}

        \begin{figure}
            \centering
            % Left column: contains two vertically stacked subfigure
            \begin{minipage}{0.5\textwidth}
                \centering
                \begin{subfigure}{\textwidth}
                    \centering
                    \includegraphics[
                        width=0.2\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{subfigure}
                \vspace{0.5em}
                \begin{subfigure}{\textwidth}
                    \centering
                    \includegraphics[
                        width=0.2\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                    \source{Author, 2024.}
                    % \label{fig:my_image}
                \end{subfigure}
            \end{minipage}
            \hfill
            % Right column: contains one larger subfigure
            \begin{minipage}{0.45\textwidth}
                \centering
                \begin{subfigure}{\textwidth}
                    \centering
                    \includegraphics[
                        width=0.6\linewidth,
                        trim=0pt 0pt 0pt 0pt, % trim: left bottom right top
                        clip
                    ]{pic/default_image.png}
                    \caption{My image.}
                \source{Author, 2024.}
                % \label{fig:my_image}
                \end{subfigure}
            \end{minipage}
            \caption{My image.}
            \source{Author, 2024.}
            % \label{fig:my_image}
        \end{figure}

        \begin{note}
            {Write your notes here}
        \end{note}
    \end{frame}

    \begin{frame}{Example 19}
        \begin{columns}[t] % Option "c" centers vertically, while "t" aligns to the top.
            \begin{column}{0.4\textwidth}
                \begin{table}[htp!]
                    \scriptsize
                    \centering
                    \caption{Node aligment (\textit{secoSanMartin}).}
                    % \label{tab:table1}
                    \begin{tabular}{rl}\toprule
                        ID 1 & ID 2 \\\midrule
                        \rowcolor{yellow}
                        13 & 13 \\
                        19 & 252 \\
                        27 & 378 \\
                        31 & 1148 \\
                        \rowcolor{yellow}
                        38 & 38 \\
                        43 & 273 \\
                        49 & 72 \\
                        \rowcolor{yellow}
                        50 & 50 \\
                        51 & 366 \\
                        53 & 241 \\
                        73 & 270 \\
                        ... & ... \\
                        1825 & 1726 \\\bottomrule
                    \end{tabular}
                \end{table}
            \end{column}
            
            \begin{column}{0.6\textwidth}
                \begin{table}[htp!]
                    \scriptsize
                    \centering
                    \caption{Node filtered \textbf{133/2072} (\textit{secoSanMartin}).}
                    % \label{tab:table1}
                    \begin{tabular}{lrrl}\toprule
                        ID & Rt & Mz & Metabolite name \\\midrule
                        13 & 6.10 & 281.05 & Unknown \\
                        38 & 6.16 & 182.10 & Unknown \\
                        50 & 6.20 & 354.10 & Unknown \\
                        128 & 6.47 & 119.20 & Unknown \\
                        184 & 6.76 & 341.14 & Unknown \\
                        261 & 7.23 & 371.06 & Unknown \\
                        272 & 7.29 & 327.01 & Unknown \\
                        314 & 7.61 & 57.02 & DECANE; EI-B; MS \\
                        336 & 7.77 & 53.96 & Unknown \\
                        339 & 7.81 & 328.01 & Unknown \\
                        400 & 8.31 & 109.08 & CITRONELLYL ACETATE... \\ %; EI-B; MS \\
                        402 & 8.31 & 109.13 & BETA-HOMOCYCLOCITRAL... \\ %; EI-B; MS \\
                        407 & 8.37 & 401.11 & Unknown \\\bottomrule
                    \end{tabular}
                \end{table}
            \end{column}
        \end{columns}

        \begin{note}
            {Introduce your self}
        \end{note}
    \end{frame}

\section*{References}
    \begin{frame}
        \begin{center}
            {\Huge\calligra Thanks!} \\
            % {\Huge\textit{Thank you for your attention!}}\\
            {\Huge $\infty$} \\
            % edwin.alvarez@pucp.edu.pe \\
            \url{https://github.com/win7}
        \end{center}
    \end{frame}
    
    % Bibliography
    % \input{ref.bib}
    \begin{frame}[allowframebreaks]
        \footnotesize % use this command to resize
        % \bibliography{ref}
        % \bibliographystyle{unsrt}
        % \bibliographystyle{alpha}
        
        \begin{itemize}
            \item Erciyes, K. (2018). \textit{Guide to graph algorithms} % (pp. 15-35). Springer International Publishing.
            \item Yao Ma, Jiliang Tang. (2021). \textit{Deep Learning on Graphs}.
            \item Stamile C., Marzullo A., Deusebio E. (2021). \textit{Graph Machine Learning}.
            \item Wu, L., Cui, P., Pei, J., Zhao, L., \& Guo, X. (2022). \textit{Graph neural networks: foundation, frontiers and applications}. % In Proceedings of the 28th ACM SIGKDD conference on knowledge discovery and data mining (pp. 4840-4841).            
            % \item Knickerbocker D. (2023). \textit{Graph Network Science with Python}.
            \item Knickerbocker, D. (2023). \textit{Network Science with Python}. % Packt Publishing Ltd.
            \item Labonne, M. (2023). \textit{Hands-On Graph Neural Networks Using Python}.
            \item Broadwater, K., \& Stillman, N. (2025). \textit{Graph neural networks in action}. % Simon and Schuster.
        \end{itemize}
    \end{frame}
\end{document}